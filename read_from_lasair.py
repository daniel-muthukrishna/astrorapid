...
def classify_lasair_light_curves(object_names=('ZTF18acsovsw',), plot=True, figdir='.'):
    ...
    # print(radec)
    # np.savez('plot_nuclear_transients2', real_tde_probs=np.array(real_tde_probs), probabilities=np.array(probabilities), names=np.array(names), radec=np.array(radec), peakmags=np.array(peakmags))
    ...
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig('astrorapid/prob2.pdf')
...
if __name__ == '__main__':
    ...
    #
    # nuc_tran_plot = np.load('plot_nuclear_transients2.npz')
    ...
    #
    # np.savetxt('/Users/danmuth/PycharmProjects/astrorapid/paper_plots/TDE_candidates.csv', np.hstack([radecs, peakmags])[probabilities > 0.4486])
    #
    # import matplotlib.pyplot as plt
    ...
    # plt.legend()
    # plt.tight_layout()
    # plt.savefig('astrorapid/prob2_unnormalised.pdf')
    #
    # plt.figure()
    # for i in range(len(real_tde_probs)):
    ...
    # plt.tight_layout()
    # plt.savefig('astrorapid/cumulativeTDE.pdf')
